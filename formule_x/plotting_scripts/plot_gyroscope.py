...
tasks = [
    {
        "file": DATA_CUT / 'Gyroscope.csv',
        "cols": ['Time (s)', 'X (rad/s)', 'Y (rad/s)', 'Z (rad/s)'],
        "output": PLOTS / "gyroscope_raw.pdf"
    }
]
...
for task in tasks:
    try:
        ...
        print("Max angular velocity:", np.sqrt(np.max(df[x_col]**2+df[y_col]**2+df[z_col]**2)))

        plt.xlabel(r'$t\;[\mathrm{s}]$')
        plt.ylabel(r'$\omega\;[\mathrm{rad}/\mathrm{s}]$')
